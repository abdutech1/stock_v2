...
import { 
  LogOut, UserCircle, Menu, X, Bell, AlertTriangle,
  LayoutDashboard, ShoppingCart, BarChart3, Package, Users, 
  Wallet, CalendarCheck, Banknote, PackageSearch, ChevronRight
} from "lucide-react";
...
export default function Navbar() {
  ...
  const links = useMemo(() => [...commonLinks, ...(isOwner ? ownerLinks : [])], [isOwner]);
  ...
  const { data: alertData, isError, isLoading } = useQuery({
  ...
  return (
    <>
      <header className={cn(
          ...
          isScrolled ? "bg-white/90 backdrop-blur-xl border-slate-200/80 shadow-sm py-2" : "bg-white border-slate-100 py-4"
      )}>
            ...
            <div className="flex items-center gap-4 min-[1200px]:gap-8 overflow-hidden">
              <Link href="/dashboard" className="flex items-center gap-2 shrink-0 group">
                ...
                <div className="hidden min-[400px]:block">
                  <span className="text-lg font-black tracking-tighter text-slate-900 uppercase italic">Jemo</span>
                  <span className="text-lg font-light tracking-tighter text-slate-500 uppercase italic ml-0.5">Boutique</span>
                </div>
              </Link>
                ...
                {links.map((link) => {
                  ...
                  return (
                    <Link key={link.href} href={link.href} className={cn(
                      ...
                      {isActive && <motion.div layoutId="nav-pill" className="absolute inset-0 bg-indigo-50/50 rounded-xl -z-10" />}
            ...
            <div className="flex items-center gap-2 sm:gap-4 shrink-0">
              {isOwner && (
                <div className="relative">
                  ...
                  <button 
  onClick={() => { setIsAlertOpen(!isAlertOpen); setIsProfileOpen(false); }}
  className={cn(
    "relative p-2 rounded-full transition-all shrink-0", // Added shrink-0
    isAlertOpen ? "text-rose-600 bg-rose-50" : "text-slate-400 hover:text-rose-600 hover:bg-rose-50"
  )}
>
  <Bell size={18} />
  ...
  {isAlertOpen && (
    <>
      {/* Backdrop overlay */}
      <div className="fixed inset-0 z-[90] bg-slate-900/10 backdrop-blur-sm sm:bg-transparent sm:backdrop-blur-none" onClick={() => setIsAlertOpen(false)} />
      ...
      >
        <div className="p-5 bg-rose-50 border-b border-rose-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] font-black uppercase text-rose-600 tracking-widest">Low Stock Alert</p>
          </div>
          <button onClick={() => setIsAlertOpen(false)} className="text-rose-400 hover:text-rose-600">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="max-h-[60vh] sm:max-h-72 overflow-y-auto divide-y divide-slate-50">
          {isLoading ? (
            ...
            <div className="p-10 text-center text-slate-300">
                ...
                <p className="text-[10px] font-black uppercase">Inventory is Healthy</p>
            </div>
          ) : lowStockItems.map((item: any) => (
            <div key={item.priceCategoryId} className="p-4 hover:bg-slate-50 transition-colors">
              <div className="flex justify-between items-center gap-3">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="w-10 h-10 rounded-lg overflow-hidden bg-slate-100 border border-slate-200 shrink-0">
                    <img 
                      src={getImageUrl(item.imageUrl || item.category?.imageUrl) || ""} 
                      alt="" 
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] font-bold text-slate-400 uppercase tabular-nums">
                      {item.fixedPrice?.toLocaleString()} ETB
                    </p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs font-black text-rose-500 tabular-nums">{item.stock?.quantity ?? 0}</p>
                  <p className="text-[8px] font-bold text-slate-300 uppercase leading-none">Left</p>
                </div>
              </div>
            </div>
          ))}
        ...
        >
          ...
          <ChevronRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
              ...
              <div className="relative">
                 <button 
                  onClick={() => { setIsProfileOpen(!isProfileOpen); setIsAlertOpen(false); }}
                  className={cn(
                    "w-9 h-9 rounded-2xl flex items-center justify-center font-black text-xs transition-all active:scale-95 shadow-md",
                    isProfileOpen ? "bg-indigo-700 ring-4 ring-indigo-50 text-white" : "bg-gradient-to-br from-indigo-500 to-indigo-700 text-white hover:shadow-indigo-100"
                  )}
                 >
                    ...
                    <>
                      ...
                      >
                        <div className="p-3 mb-1 bg-slate-50 rounded-2xl">
                          <p className="text-[10px] font-black text-slate-900 truncate uppercase">{displayName}</p>
                          <p className="text-[9px] font-bold text-indigo-500 uppercase italic tracking-wider">{role}</p>
                        </div>
                        <button onClick={() => { setIsProfileOpen(false); logout(); }} className="w-full flex items-center gap-3 p-3 text-xs font-black uppercase text-rose-500 hover:bg-rose-50 rounded-xl transition-colors">
              ...
              <button onClick={() => setIsMobileMenuOpen(true)} className="min-[1100px]:hidden p-2 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                <Menu size={22} />
              </button>
            </div>
          </div>
          ...
          <>
            ...
            >
              <div className="p-6 flex items-center justify-between border-b border-slate-50">
                <span className="text-sm font-black italic uppercase tracking-tighter">Menu</span>
                ...
                {links.map((link) => (
                  <Link 
                    ...
                    className={cn(
                      ...
                      pathname === link.href ? "bg-indigo-600 text-white shadow-lg" : "text-slate-500 hover:bg-slate-50"
                    )}
                  >
              ...
              <div className="p-6 border-t border-slate-50">
                <button onClick={logout} className="w-full flex items-center justify-center gap-2 p-4 bg-rose-50 text-rose-600 rounded-[20px] text-[11px] font-black uppercase">
